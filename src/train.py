...
def train():
    writer = SummaryWriter(
        log_dir=
        f"./runs/{model_name}/{datetime.datetime.now().replace(microsecond=0).isoformat().replace(':','-')}{'-' + os.environ['REMARK'] if 'REMARK' in os.environ else ''}"
    )
    ...
    try:
        pretrained_word_embedding = torch.from_numpy(
            np.load(r'D:\HieuDT\news-recommendation\src\data\train\pretrained_word_embedding.npy')).float()
    ...
    if model_name == 'DKN':
        try:
            pretrained_entity_embedding = torch.from_numpy(
                np.load(
                    r'D:\HieuDT\news-recommendation\src\data\train\pretrained_entity_embedding.npy')).float()
        except FileNotFoundError:
