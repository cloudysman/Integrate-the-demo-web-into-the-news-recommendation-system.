...
class NewsDataset(Dataset):
    ...
    def __init__(self, news_path):
        ...
        #print(len(self.news_parsed))
        #print("ID_head")
        #print(self.news_parsed['id'].head())
        #print("ID_tail")
        #print(self.news_parsed['id'].tail())

        if 'N55468' in self.news_parsed['id'].values:
            print("N55468 co trong new_parse")

        else:
            print("N55468 khong co trong new_parse")
...
def evaluate(model, directory, num_workers, max_count=sys.maxsize):
    ...
    Returns:
        ...
        nDCG@10
    """
    news_dataset = NewsDataset(path.join(directory, r'D:\HieuDT\news-recommendation\src\data\val\news_parsed.tsv'))
    ...
    for minibatch in tqdm(news_dataloader,
        ...
        #print("Loaded news_ids:", news_ids) 
        if "N55468" in news_ids:
            print("Processing N55468 in this minibatch.")
        if any(id not in news2vector for id in news_ids):
            news_vector = model.get_news_vector(minibatch)
            print(f"Generated news_vector: {news_vector}")
            for id, vector in zip(news_ids, news_vector):
                if id == "N55468":
                    print(f"Adding vector for N55468: {vector}")
                if id not in news2vector:
                    news2vector[id] = vector
        if "N55468" not in news2vector:
            print("N55468 is still missing after processing.")
    #     if any(id not in news2vector for id in news_ids):
    #         news_vector = model.get_news_vector(minibatch)
    #         for id, vector in zip(news_ids, news_vector):
    #             if id not in news2vector:
    #                 news2vector[id] = vector
    # if 'N53572' in news2vector:
    #     print('N53572 is in news2vector')
    # else:
    #     print('N53572 is missing from news2vector')

    news2vector['PADDED_NEWS'] = torch.zeros(
        list(news2vector.values())[0].size())

    user_dataset = UserDataset(path.join(directory, r'D:\HieuDT\news-recommendation\src\data\val\behaviors.tsv'),
    ...
    for minibatch in tqdm(user_dataloader, desc="Calculating vectors for users"):
        for news_list in minibatch["clicked_news"]:
            ...
            for x in news_list:
                if not isinstance(x, str):
                    print(f"Invalid item in news_list: {x}")
                    continue
                if x not in news2vector:
                    print(f"ID {x} not found in news2vector.")
                    continue
                news_vector = news2vector[x].to(device)
        ...
        if any(user_string not in user2vector for user_string in user_strings):
            ...
            try:
                ...
                print(f"KeyError: {e} not found in news2vector. news_list: {news_list}")
                raise e
            if model_name == 'LSTUR':
    ...
    behaviors_dataset = BehaviorsDataset(path.join(directory, r'D:\HieuDT\news-recommendation\src\data\val\behaviors.tsv'))
...
if __name__ == '__main__':
    ...
    model.eval()
    auc, mrr, ndcg5, ndcg10 = evaluate(model, r'D:\HieuDT\news-recommendation\src\data\test',
                                       config.num_workers)
    print(
        f'AUC: {auc:.4f}\nMRR: {mrr:.4f}\nnDCG@5: {ndcg5:.4f}\nnDCG@10: {ndcg10:.4f}'
    )